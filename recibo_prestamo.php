...
$id = (int) ($_GET['id'] ?? 0);
$pdo = db();
$p = getPrestamo($pdo, $id);
if (!$p) {
    flash('error', 'El préstamo no existe.');
    redirect('prestamos.php');
}
$items = getPrestamoItems($pdo, $id);
$persona = trim(($p['persona_nombre'] ?? '') . (!empty($p['persona_area']) ? ', ' . $p['persona_area'] : ''));
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<div class="no-print">
    ...
    <a class="btn-back" href="<?= h(url('prestamo.php?id=' . $p['id'])) ?>">Volver al préstamo</a>
    <a class="btn-back" href="<?= h(url('prestar.php')) ?>">Prestar otro</a>
</div>
...
<article class="hoja">
    ...
    <h1 class="doc-titulo">Recibí de préstamo de material</h1>
    <p class="doc-sub">Comprobante de entrega temporal de bienes del área de Informática</p>

    <p class="constancia">
        El área de <strong>Informática</strong> del <strong>CECYTE Baja California Sur</strong> hace constar
        que el día <strong><?= h(formatFecha($p['fecha_prestamo'], true)) ?></strong> entregó en calidad de préstamo a
        <strong><?= h($persona ?: 'la persona indicada') ?></strong> el material que se describe, con compromiso de devolución
        el <strong><?= h(formatFechaLarga($p['fecha_compromiso'])) ?></strong>. Este documento es el comprobante de salida;
        deberá conservarse hasta el regreso de los bienes.
    </p>

    <section class="seccion">
        <h2>1. Datos de quien recibe y de quien entrega</h2>
        <div class="fila">
            <div class="campo"><label>Recibe</label><span class="valor"><?= h($p['persona_nombre'] ?: '—') ?></span></div>
            <div class="campo"><label>Área o dependencia</label><span class="valor"><?= h($p['persona_area'] ?: 'No especificada') ?></span></div>
            <div class="campo"><label>Teléfono o extensión</label><span class="valor"><?= h($p['persona_telefono'] ?: 'No proporcionado') ?></span></div>
            <div class="campo"><label>Entrega · Informática</label><span class="valor"><?= h($p['prestado_por'] ?: ORG_NAME) ?></span></div>
            <div class="campo"><label>Fecha de préstamo</label><span class="valor"><?= h(formatFecha($p['fecha_prestamo'], true)) ?></span></div>
            <div class="campo"><label>Fecha de devolución</label><span class="valor"><?= h(formatFecha($p['fecha_compromiso'])) ?></span></div>
        </div>
    </section>

    <section class="seccion">
        <h2>2. Material prestado</h2>
        <table class="items">
            <thead>
                <tr>
                    <th>Tipo</th>
            ...
            <?php foreach ($items as $it): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <?php if (!empty($it['accesorios_salida'])): ?><br><?= h($it['accesorios_salida']) ?><?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
    ...
    <section class="seccion">
        ...
        <div class="fila">
            <div class="campo ancho"><label>Notas del préstamo</label><span class="valor"><?= h($p['observaciones'] ?: 'Ninguna') ?></span></div>
        </div>
    </section>
    ...
    <div class="firmas">
        <div class="firma">
            ...
            <div class="firma-linea">
                <strong><?= h($p['persona_nombre'] ?: '________________') ?></strong>
        ...
        <div class="firma">
            <div class="firma-espacio"></div>
            <div class="firma-linea">
                <strong><?= h($p['prestado_por'] ?: ORG_SHORT) ?></strong>
                <span>Nombre y firma de quien entrega · Informática</span>
            </div>
        </div>
    </div>
    ...
    <p class="pie">
        ...
        Folio <?= h($p['folio']) ?> · Generado el <?= h(date('d/m/Y H:i')) ?>.
    </p>
</article>
</body>
</html>
